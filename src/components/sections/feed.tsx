...
const FeedSection = () => {
    ...
    const initFeed = useCallback(async () => {
        const sui_provider = new JsonRpcProvider(
            process.env.NEXT_PUBLIC_RPC_URL
        );
        const aa = await sui_provider.getObject(process.env.NEXT_PUBLIC_TWITTER_ID);
        console.log(process.env.NEXT_PUBLIC_TWITTER_ID)
        console.log(aa)
        const asd = (await sui_provider.getDynamicFields(
            process.env.NEXT_PUBLIC_TWITTER_ID
        ))
        console.log(asd)
        const slotObjects = process.env.NEXT_PUBLIC_TWITTER_ID
            ? (await sui_provider.getDynamicFields(
                  process.env.NEXT_PUBLIC_TWITTER_ID
              )).data
            : [];
        console.log(slotObjects)
        let slots = await Promise.all( 
            slotObjects.map(async (slotObj: any) => {
                const rawSlot: any = await sui_provider!.getObject(
                    slotObj.objectId
                );
                ...
                <div className="feed-footer">
                    ...
                    {wallet.address === slot.edited_by ? (
                        ...
                        <Button
                            $mode="feed-button"
                            onClick={() =>
                                !isMobile() && handleBuyBoxClick(slot)
                            }
                        >
                            Buy This Box!
                        </Button>
